    ...
    const handleSubmit = async () => {
        ...
        try 
        {
          const token = await getToken(); 
            ...
            // FIX 3: Added API endpoint URL (make sure this matches your file structure!)
            // FIX 4: Added the missing comma before the headers object
            ...
            dispatch(addRating(data.rating)); // FIX 1: Updated spelling
            toast.success(data.message);
            setRatingModal(null);
        } catch (error) {
              const serverMsg = error.response?.data?.error || "Failed to submit rating";
            toast.error(serverMsg);
            
            // This prints the full error to your browser console so you can inspect it
