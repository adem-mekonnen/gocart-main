...
export default function AdminCoupons() {
    ...
    const fetchCoupons = async () => {
        // Logic to fetch coupons from the server
         try {
    ...
    const handleAddCoupon = async (e) => {
        ...
        try {
            const token = await getToken()
            newCoupon.discount = Number(newCoupon.discount) // Ensure discount is a number
            newCoupon.expiresAt = new Date(newCoupon.expiresAt) // Ensure expiresAt is a Date object
            const { data } = await axios.post("/api/admin/coupon", { coupon: newCoupon }, {
                headers: { Authorization: `Bearer ${token}` }
            })
            toast.success(data.message || "Coupon added successfully")
            await fetchCoupons() // Refresh the coupon list after adding
        } catch (error) {
    ...
    const deleteCoupon = async (code) => {
        ...
        try {
            ...
            const { data } = await axios.delete(`/api/admin/coupon?code=${code}` , {
             ...
             await fetchCoupons() // Refresh the coupon list after deletion
